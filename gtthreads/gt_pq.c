...
static inline void __add_to_runqueue_cfs(rbtree runq, uthread_struct_t *u_elem)
{
	...
	runq->tree_sum_nice_values_weight += prio_to_wmult[u_elem->nice_value];	
	long delta = u_elem->vruntime - runq->min_vruntime;
	if (delta > 0)
	u_elem->vruntime = delta;
...
extern uthread_struct_t *sched_find_best_uthread(kthread_runqueue_t *kthread_runq)
{
	...
	rbtree_node temp_rb_node;

	if (!(temp_rb_node = find_left_node(cfs_rq->root)))
	{
		gt_spin_unlock(&(kthread_runq->kthread_runqlock));
		return NULL;
	}

	u_obj = (uthread_struct_t*) temp_rb_node->value;
...
extern rbtree_node find_leftmost_node (rbtree t)
{
	rbtree temp;
	temp = t;
	if (temp->root)
...
extern rbtree_node find_left_node (rbtree_node t)
{
	if (t == NULL)
	return NULL;
	
	if(t->left != NULL)
	find_left_node(t->left);
